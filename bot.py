    ...
    if administrador:
        ...
        if '/decrypt' in text:
            ...
            if getUser:
                ...
                try:
                    ...
                    bot.editMessageText(chat_id=chat,message_id=sms.message_id,text="⚠️EL PROXY NO ES VÁLIDO❗️")
                    print(proxy_de)
                    return
                try:
                    proxy_ip = str(proxy_de).split(':')[0]
                    proxy_port = str(proxy_de).split(':')[1]
                    bot.editMessageText(chat_id=chat,message_id=sms.message_id,text=f'🧬Desencriptado Realizado:\n\nIP : {proxy_ip}\nPUERTO : {proxy_port}')
                except:
                    bot.editMessageText(chat_id=chat,message_id=sms.message_id,text="⚠️EL PROXY NO ES VÁLIDO❗️")
                    print(proxy_de)
                return
        ...
        if '/fz_find' in text:
            ...
            if getUser:
                ...
                except Exception as ex:bot.sendMessage(chat,f"✖️HA OCURRIDO UN ERROR INESPERADO✖️\n\n{str(ex)}")
        ...
        if '/add' in text:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(chat,'✖️Error en el comando /add username✖️')
            elif getUser:
        ...
        if '/ban' in text:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(chat,'✖️Error en el comando /ban username✖️')
            elif getUser:
                bot.sendMessage(chat,'✖️No Tiene Permiso✖️')
            return
...
if __name__ == '__main__':
    if dev == 1 :
        administrador = 'TuguerX'
        bot_token = '5416718139:AAGMElSpymewT2Mn_txoLay7eDl3huDOVYQ'
        user_id = 1307228755
    else :
